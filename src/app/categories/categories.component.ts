...
export class CategoriesComponent implements OnInit {
  ...
  uploadImage(){
    this.spinnerService.show();
    this.loginService.IsCategoryExist(this.authForm.value).subscribe((response) => {
      if(response.status == 1){
        ...
        this.loginService.UploadImage(payload).subscribe((response) => {
          ...
          this.loginService.addCategory(this.authForm.value).subscribe((response) => {
            if(response.status == 1){
  ...
  updateImage(){
    if(this.Image != ''){
      ...
      this.loginService.IsCategoryExist(dict).subscribe((response) => {
        if(response.status == 1){
          var payload = new FormData();
          payload.append('image', this.selectedFileProfile, this.selectedFileProfile.name);
          this.loginService.UploadImage(payload).subscribe((response) => {
